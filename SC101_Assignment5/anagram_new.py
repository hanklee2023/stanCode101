...
def main():
    ...
    while True:
        ...
        if key == EXIT:
            ...
            # print("--- %s total seconds ---" % (end - start_time))
...
def read_dictionary(s):
    ...
    :param s            :(str) Key word to change original dict
    """
    global dict
    ...
    with open(FILE, 'r') as r:
        for item in r:
            ...
            # full lst
            if item[0] in s:
                if len(item[0:len(item)-1]) == length:
                    full_lst[item] = True
    ...
    for voc in full_lst:
        if len(voc[0:len(item) - 1]) > 7:
            dict[voc[0]][voc[0:2]][voc[0:3]][voc[0:4]][voc[0:5]][voc[0:6]][voc[0:7]][voc[0:len(item) - 1]] = 1
...
def find_anagrams(s, found_dict):
    ...
    print("--- %s function seconds ---" % (end - start_time))
...
def find_diff_char(s1, s2):
    """
    This function is find whether any string in dict contains sub string
    :param s1   :(str) subset of overall string
    :param s2   :(str) full subset of overall string
    :return     :(str) the diff char between 2 sets
    """

    return list((Counter(s1) - Counter(s2)).elements())
